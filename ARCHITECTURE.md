...
anyone ──executeExit(agent)──▶ mark ≥ tp | ≤ sl | age ≥ MAX_HOLD → WETH → USDC (fill ≥ mark·0.98), settle
   ...
   ladder (after settle): mult ∈ {1,2,4,8} from cumPnl ≥ SCALE_T*_BPS·base, grossProfit/grossLoss ≥ SCALE_PF_T*_BPS,
           trades ≥ SCALE_MIN_TRADES (sample floor), cumPnl ≥ base·max(0, spot/benchPrice − 1)   [beats buy-and-hold]
